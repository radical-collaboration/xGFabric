...
from ..common.cfd_common import (  # noqa: E402
...
@dataclass
class FNO_Config:
    iter_min = None  # Minimum iteration number (for multi-iteration format)
    iter_max = None  # Maximum iteration number (for multi-iteration format)
    ...
    target_file_epochs = None  # Adjust epochs so total files × epochs ≈ target; overrides epochs when set
...
class SpectralConv2d(layers.Layer):
    def __init__(self, in_ch, out_ch, modes1, modes2, **kwfno_config):
        super().__init__(**kwfno_config)
        self.modes1, self.modes2 = modes1, modes2
        scale = 1.0 / (in_ch * out_ch)
        self.wr = self.add_weight(
            name="wr",
            shape=(modes1, modes2, in_ch, out_ch),
    ...
    def _compl_mul(self, x_r, x_i):
        out_r = tf.einsum("bmni,mnio->bmno", x_r, self.wr) - tf.einsum(
            "bmni,mnio->bmno", x_i, self.wi
        )
        out_i = tf.einsum("bmni,mnio->bmno", x_r, self.wi) + tf.einsum(
            "bmni,mnio->bmno", x_i, self.wr
        )
        return out_r, out_i
...
class FNOBlock(layers.Layer):
    def __init__(self, hidden, modes1, modes2, **kwfno_config):
        super().__init__(**kwfno_config)
...
class FNO2D(tf.keras.Model):
    def __init__(self, in_ch, out_ch, hidden, n_layers, modes1, modes2, **kwfno_config):
        super().__init__(**kwfno_config)
        self.lift = layers.Dense(hidden)
        self.blocks = [
            FNOBlock(hidden, modes1, modes2, name=f"fno_block_{i}")
...
def fno_main_entry(file_pairs, output_directory, fno_config: FNO_Config, log):
    ...
    ITER_MIN = fno_config.iter_min or 0
    ...
    BATCH = fno_config.batch
    LR = fno_config.learning_rate
    PATIENCE = fno_config.patience
    CLIP = 1.0

    log.info(
        f"Grid: {NX}x{NY}  modes=({MODES1},{MODES2})  hidden={HIDDEN}  layers={N_LAYERS}"
    )
    ...
    if fno_config.target_file_epochs is not None:
        n_files = len(file_pairs)
        fno_config.epochs = max(10, fno_config.target_file_epochs // n_files)
        log.info(
            f"target_file_epochs={fno_config.target_file_epochs}: "
            f"{n_files} files → {fno_config.epochs} epochs"
        )

    EPOCHS = fno_config.epochs
    ...
    log.info(
        f"WS range : {min(WS_values):.1f} – {max(WS_values):.1f} m/s ({len(WS_values)} unique)"
    )
    ...
    def sinusoidal_embed(param_val, x_grid, y_grid, L):
        ...
        for k in range(1, L + 1):
            channels.append(
                (param_val * np.sin(k * 2 * np.pi * x_grid)).astype(np.float32)
            )
            channels.append(
                (param_val * np.cos(k * 2 * np.pi * x_grid)).astype(np.float32)
            )
            channels.append(
                (param_val * np.sin(k * 2 * np.pi * y_grid)).astype(np.float32)
            )
            channels.append(
                (param_val * np.cos(k * 2 * np.pi * y_grid)).astype(np.float32)
            )
    ...
    assert (
        X_fno.shape[-1] == IN_CH
    ), f"IN_CH mismatch: expected {IN_CH}, got {X_fno.shape[-1]}"
    ...
    lr_sched = tf.keras.optimizers.schedules.CosineDecay(
        LR, decay_steps=EPOCHS * n_batches
    )
    ...
    for ep in range(EPOCHS):
        ...
        for xb, yb in ds_tr:
            ...
            grads, _ = tf.clip_by_global_norm(
                tape.gradient(tr_loss, fno.trainable_variables), CLIP
            )
        ...
        avg_vl = float(
            np.mean([float(loss_fn(yb, fno(xb, training=False))) for xb, yb in ds_vl])
        )
        ...
        if ep % 10 == 0 or ep == EPOCHS - 1:
            ...
            log.info(
                f"epoch {ep:4d}  train={avg_tr:.4e}  val={avg_vl:.4e}  best={best_val:.4e}  pat={pat}/{PATIENCE}"
            )
    ...
    pred_u_ms = pred_te[..., 0] * VEL_MAX
    true_u_ms = true_te[..., 0] * VEL_MAX
    pred_v_ms = pred_te[..., 1] * VEL_MAX
    true_v_ms = true_te[..., 1] * VEL_MAX
    ...
    ax.set_title(
        f"FNO2D scatter (test)  RMSE={rmse_spd:.3f} m/s\niters [{ITER_MIN}, {ITER_MAX}]"
    )
    ax.set_aspect("equal")
    ax.legend()
    ax.grid(alpha=0.3)
    ...
    fig.suptitle(
        f"FNO2D field comparison  |  iters [{ITER_MIN}, {ITER_MAX}]", fontsize=13
    )
    fig.tight_layout()
