...
def expand_to_df(ws, file):
    try:
        ...
        if n_dropped > 0:
            logging.warning(
                f"Dropped {n_dropped}/{n_before} rows with empty/invalid values"
            )
        if df.empty:
            logging.warning(f"File has no valid rows after cleaning")
            return None, None

        # Calculate velocity magnitude if not present
        if "U_Magnitude" not in df.columns:
            df["U_Magnitude"] = (
                df["U_0"] ** 2 + df["U_1"] ** 2 + df["U_2"] ** 2
            ) ** 0.5
...
def read_cfd_data(filepath_or_dir, pattern="cups_structure_ws*.csv"):
    ...
    if os.path.isdir(filepath_or_dir):
        ...
        if not files:
            ...
            if files:
                logging.info(
                    f"Found {len(files)} files matching new naming pattern 'sim_*.csv'"
                )
        else:
    ...
    for file in files:
        try:
            ...
            # Drop rows where any critical column is NaN
            critical = [
                c for c in ("x", "y", "z", "U_0", "U_1", "U_2") if c in df.columns
            ]
            n_before = len(df)
            df = df.dropna(subset=critical)
            n_dropped = n_before - len(df)
            if n_dropped > 0:
                logging.warning(
                    f"Dropped {n_dropped}/{n_before} rows with empty/invalid values in {filename}"
                )
            if df.empty:
                logging.warning(
                    f"File has no valid rows after cleaning, skipping: {filename}"
                )
                continue
...
def filter_zero_velocity(df, umag_min=0.10):
    ...
    if "U_Magnitude" not in df.columns:
        if all(c in df.columns for c in ["U_0", "U_1", "U_2"]):
            ...
            logging.warning(
                "No U_Magnitude or velocity components found; skipping velocity filter"
            )
            return df
