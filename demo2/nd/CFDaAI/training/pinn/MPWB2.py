import os, sys, argparse, json, random, logging
...
def piecewise_dir_weight(epoch, warm_start, warm_end, decay_start, decay_end, final_ratio):
...
def inflow_gates_from_uv(u, v, thresh=0.3, k=8.0):
    ...
    print(f"[gate diag] mean gates: N={float(gN.mean()):.2f} S={float(gS.mean()):.2f} "
...
def pinn_loss(
    ...
    l_b_dir = cosine_dir_loss(Ub, Vb, u_b, v_b, weights_b,
        speed_min=dir_loss_speed_min)
    ...
    data_loss = (l_b + center_lambda*l_c) \
        + (w_dir_boundary*l_b_dir) + (center_lambda*w_dir_center*l_c_dir) \
        + (w_speed_boundary*l_b_spd) + (center_lambda*w_speed_center*l_c_spd) \
        + (l_b_mag + center_lambda*l_c_mag)
    total = alpha * physics_loss + (1.0 - alpha) * data_loss
    del tape1, tape2
    return total, physics_loss, data_loss, {"fu": f_u, "fv": f_v, "div": div,
                                            "l_b": l_b, "l_c": l_c, "l_b_dir": l_b_dir, "l_c_dir": l_c_dir}
...
def train_step(model, opt,
               ...
               u_b, v_b, w_b,
               Lx, Ly, Lt, nu,
               alpha, w_div, add_data, center_lambda, 
               w_dir_boundary, w_dir_center, dir_loss_speed_min, 
               w_speed_boundary, w_speed_center ):
