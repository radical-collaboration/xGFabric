...
class PilotController(object):
    ...
    def _submit_pilot(self, nodes=1, runtime=600):
        ...
        pd.nodes    = nodes
        pd.runtime  = runtime
        pd.resource = self._resource_description.get("resource_type",
                                                     'local.localhost')
    ...
    def start_initial_pilot(self, expected_workload=None):
        ...
        :param   expected_workload: Dictionary describing expected data and
                 workload characteristics.
        :return: UID of the submitted pilot, or None if no pilot is started.
        """

        if self._pilots:
            # A pilot is already running, avoid redundant submission.
            return

        if expected_workload and expected_workload.get("size", 0) > 0:

            # Ensure reasonable runtime
            runtime = min(self._resource_description.get("max_runtime", 600), 600)

            # Adjust nodes dynamically
            nodes   = min(self._resource_description.get("nodes", 1),
                          max(1, expected_workload["size"] // 10))
    ...
    def start_pilot(self, data):
        ...
        required_nodes = max(1, data_size // 10)

        # Determine needed resources dynamically
        ...
        runtime = min(self._resource_description.get("max_runtime", 600), 600)
        nodes_to_allocate = min(self._resource_description.get("nodes", 1),
                                required_nodes)
